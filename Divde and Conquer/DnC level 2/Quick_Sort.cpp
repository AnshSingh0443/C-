...
int partition(int arr[], int s, int e) {
  ...
  swap(arr[pivotIndex], arr[rightIndex]);//placing it 
  ...
  while( i < pivotIndex && j > pivotIndex) {
        ...
        //i->will be either at element to be swapped(case A) or at pivot index(case B)
        while( arr[j] > pivotElement){
        j--;
        }
        //j->will be either at element to be swapped(case A) or at pivot index(case B)
